...
#
#   CORES=0-3 GAME=chess WIDTH=256 DEPTH=8 ACTORS="16 32 64" bash scripts/measure_states.sh
set -uo pipefail
cd "$(dirname "$0")/.."
...
for actors in $ACTORS; do
  ...
  taskset -c "$CORES" "$BIN" --game="$GAME" --path="$out" \
    ...
    --learning_rate=0.0001 --weight_decay=0.0001 --checkpoint_freq=1000000 \
...
print(f"{out.split('/')[-1]}  states/s={states / w:7.1f}  (games={games}, avg_len={states / max(games, 1):.0f})  rows/s={rows_s:8.1f}  rows_per_state={rows_s / max(states / w, 1e-9):.0f}")
PYEOF
done
