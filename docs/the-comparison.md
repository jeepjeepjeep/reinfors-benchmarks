...
n512×2 compiled** — these are the topologies encoded in `v1_training.json` (decision
point: revise them there if the grids disagree). OpenSpiel's optimum caps nothing —
half-fill helped only below the optimal call size, so its inference batch equals the
actor count. The reinfors configuration includes its
